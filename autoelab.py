...
def getanswers(questionid):
    global answers
    try:
        answers = json.loads(requests.get(answerdb + questionid + '.json').text)['files']
...
def alreadysolved():
    ...
    if len(code) > 3:
        ...
        if result['error'] == 0 and result['score'] == 100:
            return(True)
...
def reportgenerator(questionno):
    ...
    outfile = 'Reports\\L0{}Q{}.jpg'.format(level, str(questionno).rjust(2,'0'))
...
def getinput():
    ...
    if campuscode == '':
        campuscode = input('Campus (ktr|rmp|vdp) : ')

    department = config['department']
    if department == '':
        department = input('Department (cse|it) : ')

    skill = config['skill']
    if skill == '':
        skill = input('Skill (java|daa): ')
    ...
    start = config['start']
    if start == '':
        start = input('Starting Question Number (0-99) : ')

    end = config['end']
    if end == '':
        end = input('Ending Question Number (0-99) : ')
...
def autoelab(username, password, campuscode, department, skill, level, start, end, delay, generatereports=False):
    ...
    with requests.Session() as session:
        ...
        for questionno in range(start, end):
            ...
            if solved == False:
                ...
                print(' - Solved')
                if generatereports:
                    reportgenerator(questionno)
                if not delay == 0:
                    totaldelay = delay + random.randint(0,10)
...
if len(sys.argv) == 10:
    ...
    sample = json.loads(requests.get('https://raw.githubusercontent.com/autoelab/autoelab/master/config.json').text)
    with open('config.json', 'w') as f:
        json.dump(sample, f, indent=4)
    input(
        'Welcome! A sample config.json file has been created.\n' + 
        'Edit it, add the required information and try again.\n' + 
